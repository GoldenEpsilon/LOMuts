...
#define stack_text
	return "You can only take @wdamage@s#up to @w2/" + string(skill_get(mod_current)+3) + "@s your max @rHP@s per hit";
...
#define custom_step1
with Player {
	steelNerves = (maxhealth * 2)/(2+skill_get(mod_current));
	tempsteelNerves = 0;
	OldHealth = my_health;
...
with Player {
	if("steelNerves" in self && "changedCanDie" in self && "OldHealth" in self){
		if (my_health < OldHealth){
			if(my_health < ceil(OldHealth-(steelNerves + tempsteelNerves)) && OldHealth > floor(OldHealth-(steelNerves + tempsteelNerves))){
				my_health = min(
					max(
						max(my_health, floor(OldHealth-max(steelNerves + tempsteelNerves, 1))),
						min(max(1, my_health+1), OldHealth-1)
					),
				maxhealth);
				nexthurt += 6;
				if(my_health > 0) {
		...
		var nervesTemp = OldHealth - my_health;
		tempsteelNerves -= nervesTemp;
		if(fork()){
			wait(2);
