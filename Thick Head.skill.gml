...
#define skill_avail
	return skill_get_at(6) != null && (!instance_is(self, CustomObject) && !instance_is(self, CustomProp)) || instance_exists(Loadout);
...
#define skill_create(_skill, _num)
	with(instance_create(0, 0, SkillIcon)) {
		...
		if(is_string(_skill) && mod_exists("skill", _skill)){
			...
			var texts = ["haha","+4 @rMAX HP@s","MORE @wSPEED@s#WALK NORMAL ON ALL TERRAIN","ATTRACT @wDROPS@s AND @gRADS@s FROM FURTHER","MORE @rHP@s AND @yAMMO@s DROPS","@wTHRONE BUTT@s","SOME KILLS REGENERATE @yAMMO@s","SOME KILLS REGENERATE @rHP@s","@wENEMIES@s TOUCHING YOU TAKE DAMAGE","MORE @rHP@s FROM MEDKITS","HIGHER @yAMMO@s MAX","LESS ENEMY @rHP@s","SLOWER @wENEMY BULLETS@s","MORE @wMELEE@s RANGE","@wNO DAMAGE@s FROM EXPLOSIONS AND FIRE#WHEN UNDER 4 @rHP@s","@wSHELLS@s BOUNCE FURTHER","MOST HIT @wBULLETS@s BECOME @yAMMO@s","@wENERGY WEAPONS@s DEAL MORE DAMAGE","GET @rFULL HP@s AND SOME @yAMMO@s","BETTER @wACCURACY@s","@wCORPSES@s FLY & HIT HARDER","HOMING @wBOLTS@s","HIGHER @wRATE OF FIRE@s#AS @rHP@s GETS LOWER","@wKILLS@s LOWER YOUR @wRELOAD TIME@s","@wDAMAGE@s TAKEN IS DEALT TO#ALL @wENEMIES@s ON SCREEN","@gMUTATE@s LATER","PUSH THROUGH#A LIMITED NUMBER OF @wWALLS@s","PREVENTS @wDEATH@s ONCE#RECHARGES AT FULL @rHP@s IN THE NEXT AREA","EXTRA @wCHEST@s SPAWN","MORE WEAPON DROPS"]
			text = texts[_skill];
		}
	}